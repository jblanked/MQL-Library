...
private:
   ...
   bool              isExpired(const string key)
   {
      ...
      {
         // failed to read
         // file doesnt exist more than likely
         // erase and force fetch
         this.erase(key);
         return true;
      }

      this.tempValue = this.json[this.tempKey]["timeout"].ToStr();
      if(StringToTime(tempValue) == 0)
      {
         // time is 0
         // not set? or parsing issue?
         // erase and force fetch
         this.erase(key);
         return true;
      }
      ...
      // not at timeout yet
      if(this.TimeCurrent() < StringToTime(tempValue)) return false;

      // timeout reached, erase file
      this.erase(key);
      return true;
   }
